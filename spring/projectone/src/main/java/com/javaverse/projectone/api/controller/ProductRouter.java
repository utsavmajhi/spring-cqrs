...
import com.javaverse.projectone.api.config.filter.ProductHandlerFilterFunction;
...
public class ProductRouter {
    ...
    @Bean
    public RouterFunction<ServerResponse> productRouterFunction(ProductHandler handler, ProductHandlerFilterFunction filter) {
        return route()
                .path("/products", builder -> builder
                        .GET("/events/sse", accept(APPLICATION_STREAM_JSON), handler::events)
                        .GET("/{id}", handler::get)
                        .GET("", accept(APPLICATION_JSON), handler::findAll)  // todo sse
                        .POST("", accept(APPLICATION_JSON), handler::save)
                        .PUT("", accept(APPLICATION_JSON), handler::update)
                        .DELETE("", accept(APPLICATION_JSON), handler::delete)
                )
